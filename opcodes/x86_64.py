...
class ISAExtension:
    ...
    :ivar name: name of the ISA extension. Possible values are:
        ...
        - "AMX-TILE"         := Advanced Vector eXtension TILE management instructions.
        ...
        - "CET-IBT"          := The `ENDBR64` instruction and `NOTRACK` prefix for `CALL`/`JMP` instructions.
    """
...
class ModRM:
    ...
    :ivar rm: a register, a memory operand, or an opcode extension. Possible values are an int value, or a reference \
        to an instruction operand.

        If rm is an int value, this value extends the opcode and must be directly coded in the reg field.
